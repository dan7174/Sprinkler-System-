...
Phases 1-5 are implemented: source manifest, Rain Bird resource-family index, JSON Schemas for intake/water tests/products/zones/projects, a starter Rain Bird product catalog with published performance tables, a tested calculation library — units, velocity, Hazen-Williams friction loss, elevation, critical pressure path, precipitation rate, matched precipitation, ET-based scheduling with cycle-and-soak, pump TDH/curve/NPSH checks, valve-wire voltage drop and sizing, and basic drainage screening. See [docs/limitations.md](docs/limitations.md) for exactly what exists and what does not.

The design engine (intake review, hydrozoning, compatibility checks, product selection, risk tracking) and the deliverable generators (design report, zone schedule, bill of materials, SVG visual plan, commissioning and maintenance outputs) are in place with 210 passing tests, including an end-to-end sample-project integration test. The audit engine adds troubleshooting decision trees, existing-product identification and replacement compatibility checks. Automated head layout, PDF/DXF drawing export and the Silverton pilot project still need to be implemented and validated.
